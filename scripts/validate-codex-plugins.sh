...
if [[ -n "$unexpected_plugin_configs" ]]; then
  echo "Unexpected plugin configs for skills-only release:" >&2
  echo "$unexpected_plugin_configs" >&2
  exit 1
fi

skill_count="$(find "$ROOT/plugins" -name SKILL.md | wc -l | tr -d '[:space:]')"
if [[ "$skill_count" != "17" ]]; then
  fail "Expected 17 packaged skills, found $skill_count"
fi

reference_count="$(find "$ROOT/plugins" -path '*/references/*' -type f | wc -l | tr -d '[:space:]')"
if [[ "$reference_count" != "148" ]]; then
  fail "Expected 148 packaged reference files, found $reference_count"
fi
